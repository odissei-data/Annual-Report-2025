  ...
  <div class="content-page flow">
    ...
    <div class="budget-box">
      ...
      <table class="budget-table">
        ${ws.bpItems.map(item => `<tr><td>${item.label}</td><td class="budget-amt">${item.val}</td></tr>`).join('\n        ')}
      </table>
    </div>
  </div>`;
}
...
:root {
  ...
  --serif: 'Roboto Slab', Georgia, serif; --sans: 'Ubuntu', system-ui, sans-serif;
}
...
.cover { width: 210mm; height: 297mm; position: relative; overflow: hidden; background: #001e2c; display: flex; flex-direction: column; justify-content: space-between; page-break-after: always; }
...
.cover-stat { flex: 1; padding: 6mm 10mm; border-right: 1px solid rgba(255,255,255,0.08); }
.cover-stat:last-child { border-right: none; }
.cover-stat-num { font-family: var(--serif); font-size: 18pt; font-weight: 700; color: #fff; line-height: 1; }
.cover-stat-label { font-size: 6pt; letter-spacing: 0.1em; text-transform: uppercase; color: rgba(255,255,255,0.4); margin-top: 1mm; }

.content-page { width: 210mm; min-height: 297mm; padding: 16mm 18mm 14mm; page-break-before: always; page-break-inside: avoid; position: relative; }
...
.section-num { font-family: var(--serif); font-size: 60pt; font-weight: 700; color: var(--light); position: absolute; top: 10mm; right: 14mm; line-height: 1; }
h2 { font-family: var(--serif); font-size: 22pt; font-weight: 700; color: var(--ink); line-height: 1.15; margin-bottom: 4mm; }
h3 { font-family: var(--serif); font-size: 11pt; font-weight: 700; color: var(--ink); line-height: 1.3; margin-bottom: 2mm; }
...
.chair-grid { display: grid; grid-template-columns: 52mm 1fr; gap: 10mm; margin-top: 5mm; }
.chair-photo { width: 100%; height: 68mm; object-fit: cover; object-position: top; border-radius: 2px; }
...
.pullquote p { font-family: var(--serif); font-size: 9pt; font-style: italic; color: var(--brand); line-height: 1.6; margin: 0; }

.boards-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 6mm; margin-top: 5mm; }
.board-panel { padding: 4mm; background: var(--light); border-radius: 2px; page-break-inside: avoid; }
.person-list li { display: flex; justify-content: space-between; padding: 1.2mm 0; border-bottom: 1px solid var(--rule); font-size: 7.5pt; gap: 4mm; }
...
.ws-lead { font-size: 8.5pt; color: var(--muted); line-height: 1.7; margin-bottom: 5mm; border-left: 2px solid var(--teal); padding-left: 3mm; }
.ws-body { margin-bottom: 5mm; }
.ws-body p { font-size: 8.5pt; line-height: 1.7; color: var(--ink); margin-bottom: 2.5mm; }
...
.papers-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 6mm; margin-top: 5mm; align-items: start; }
.paper-card { border-top: 2px solid var(--teal); padding-top: 4mm; page-break-inside: avoid; }
...
.paper-card h3 { font-size: 9pt; margin-bottom: 2.5mm; }
.paper-card p { font-size: 7.5pt; line-height: 1.6; color: var(--muted); }
.paper-ref { font-size: 7pt; font-style: italic; color: var(--muted); margin-top: 3mm; border-top: 1px solid var(--rule); padding-top: 2mm; }
...
.fin-summary { display: grid; grid-template-columns: repeat(5, 1fr); gap: 3mm; margin-top: 5mm; margin-bottom: 6mm; }
...
.kpi-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 6mm; margin-top: 5mm; }
...
<div class="cover">
  ...
  <div class="cover-stats">
    ${heroStats.map(s => `<div class="cover-stat"><div class="cover-stat-num">${fmtNum(s.num)}${s.suffix}</div><div class="cover-stat-label">${s.label}</div></div>`).join('')}
  </div>
</div>
...
<div class="content-page flow">
  ...
  <div class="boards-grid">
    ${boards.map(b => `<div class="board-panel"><h4>${b.title}</h4><ul class="person-list">${b.people.map(p => `<li><span class="person-name">${p.name}</span><span class="person-org">${p.org}</span></li>`).join('')}</ul></div>`).join('')}
  </div>
</div>
...
<div class="content-page flow">
  ...
  <div class="papers-grid">
    ${papers.map(p => `<div class="paper-card"><p class="paper-field">${p.field}</p><h3>${p.title}</h3><p>${p.body}</p><p class="paper-ref">${p.ref}</p></div>`).join('')}
  </div>
</div>
...
<div class="content-page flow">
  ...
  <div class="testi-grid">
    ${testis.map(t => `<div class="testi-card"><div class="testi-quote">&ldquo;</div><p>${t.quote}</p><div class="testi-by"><img class="testi-avatar" src="${t.avatar}" alt="${t.name}"><div><div class="testi-name">${t.name}</div><div class="testi-role">${t.role}</div></div></div></div>`).join('')}
  </div>
</div>
...
<div class="content-page flow">
  ...
  <div class="fin-summary">
    ${finCells.map(c => `<div class="fin-cell${c.isTotal ? ' total' : ''}"><div class="fin-name">${c.name}</div><div class="fin-amt">${c.amt}</div><div class="fin-desc">${c.desc}</div></div>`).join('')}
  </div>
...
<div class="content-page flow">
  ...
  <div class="kpi-grid">
    ${kpiCells.map(c => `<div class="kpi-cell"><h4>${c.title}</h4><ul class="kpi-list">${c.items.map(i => `<li>${i}</li>`).join('')}</ul></div>`).join('')}
  </div>
</div>
