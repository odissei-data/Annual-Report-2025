...
['#data', '#observatory', '#laboratory', '#hub'].forEach(id => {
  ...
  const paras = [];
  s.find('.ws-text p').each((i, el) => { paras.push($(el).html()); });
  ...
  workstreams.push({ num, title, lead, paras, bpTotal, bpLabel, bpItems });
});
  ...
  <div class="ws-inner">
    ...
    <div class="budget-box">
      ...
      <table class="budget-table">
        ${ws.bpItems.map(item => `
        <tr><td>${item.label}</td><td class="budget-amt">${item.val}</td></tr>`).join('')}
      </table>
    </div>
  </div>
</div>`;
...
<style>
:root {
  --brand: #006489; --teal: #00b1b7; --ink: #0d1e2c; --dark: #00415a;
  --muted: #5a7282; --rule: #c8dce8; --light: #eaf2f6;
...
html, body { font-family: var(--sans); font-size: 9pt; line-height: 1.6; color: var(--ink); background: white; }
img { display: block; max-width: 100%; }
p { margin-bottom: 0.5em; } p:last-child { margin-bottom: 0; }
ul { list-style: none; }

.cover {
  width: 210mm; height: 297mm; position: relative; overflow: hidden;
  background: #001e2c; display: flex; flex-direction: column; justify-content: space-between;
...
h2 { font-family: var(--serif); font-size: 20pt; font-weight: 700; color: var(--ink); line-height: 1.15; margin-bottom: 3mm; }
h3 { font-family: var(--serif); font-size: 12pt; font-weight: 700; color: var(--ink); line-height: 1.3; margin-bottom: 2mm; }
h4 { font-size: 7.5pt; font-weight: 700; letter-spacing: 0.1em; text-transform: uppercase; color: var(--brand); margin-bottom: 2mm; border-bottom: 1.5px solid var(--teal); padding-bottom: 1.5mm; }

.chair-grid { display: grid; grid-template-columns: 52mm 1fr; gap: 8mm; margin-top: 4mm; }
.chair-photo { width: 100%; height: 65mm; object-fit: cover; object-position: top; border-radius: 2px; }
.chair-name { font-family: var(--serif); font-size: 10pt; font-weight: 700; color: var(--ink); margin-top: 2mm; }
.chair-role { font-size: 7pt; color: var(--muted); line-height: 1.5; }
.pullquote { border-left: 3px solid var(--teal); padding: 2mm 4mm; margin: 3mm 0; background: var(--light); border-radius: 0 2px 2px 0; }
.pullquote p { font-family: var(--serif); font-size: 9pt; font-style: italic; color: var(--brand); line-height: 1.55; margin: 0; }

.boards-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 6mm; margin-top: 4mm; }
.board-panel { padding: 3.5mm; background: var(--light); border-radius: 2px; }
.person-list li { display: flex; justify-content: space-between; align-items: baseline; padding: 1mm 0; border-bottom: 1px solid var(--rule); font-size: 7pt; gap: 4mm; }
.person-list li:last-child { border-bottom: none; }
.person-name { font-weight: 500; color: var(--ink); }
.person-org { color: var(--muted); font-size: 6.5pt; text-align: right; }

.ws-label { font-size: 7pt; font-weight: 700; letter-spacing: 0.15em; text-transform: uppercase; color: var(--teal); margin-bottom: 2mm; }
.ws-title { font-family: var(--serif); font-size: 18pt; font-weight: 700; color: var(--ink); line-height: 1.15; margin-bottom: 3mm; }
.ws-lead { font-size: 8.5pt; color: var(--muted); line-height: 1.65; margin-bottom: 4mm; border-left: 2px solid var(--teal); padding-left: 3mm; }
.ws-body { margin-bottom: 5mm; }
.ws-body p { font-size: 8.5pt; line-height: 1.7; color: var(--ink); margin-bottom: 3mm; }
.ws-body p:last-child { margin-bottom: 0; }

.budget-box { background: var(--dark); padding: 5mm; border-radius: 2px; }
.budget-header { display: flex; justify-content: space-between; align-items: baseline; margin-bottom: 1mm; }
.budget-label { font-size: 6pt; letter-spacing: 0.12em; text-transform: uppercase; color: rgba(255,255,255,0.45); }
.budget-total { font-family: var(--serif); font-size: 16pt; font-weight: 700; color: #fff; }
.budget-sublabel { font-size: 6.5pt; color: rgba(255,255,255,0.4); margin-bottom: 2.5mm; }
.budget-table { width: 100%; border-collapse: collapse; }
.budget-table tr { border-top: 1px solid rgba(255,255,255,0.1); }
...
.papers-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 6mm; margin-top: 4mm; }
.paper-card { border: 1px solid var(--rule); padding: 4mm; border-radius: 2px; }
.paper-field { font-size: 6.5pt; font-weight: 700; letter-spacing: 0.15em; text-transform: uppercase; color: var(--teal); margin-bottom: 1.5mm; }
.paper-card h3 { font-size: 9pt; margin-bottom: 1.5mm; }
.paper-card p { font-size: 7.5pt; line-height: 1.6; color: var(--muted); }
.paper-ref { font-size: 6.5pt; font-style: italic; color: var(--muted); margin-top: 2mm; border-top: 1px solid var(--rule); padding-top: 1.5mm; }

.testi-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 6mm; margin-top: 4mm; }
.testi-card { background: var(--light); padding: 4mm; border-radius: 2px; }
.testi-quote { font-family: var(--serif); font-size: 28pt; font-weight: 700; color: var(--teal); line-height: 0.8; margin-bottom: 2mm; }
.testi-card p { font-size: 7.5pt; line-height: 1.6; color: var(--ink); font-style: italic; }
.testi-by { display: flex; align-items: center; gap: 3mm; margin-top: 3mm; }
.testi-avatar { width: 32px; height: 32px; border-radius: 50%; object-fit: cover; }
.testi-name { font-size: 7.5pt; font-weight: 700; color: var(--ink); }
.testi-role { font-size: 6.5pt; color: var(--muted); }

.fin-summary { display: grid; grid-template-columns: repeat(5, 1fr); gap: 3mm; margin-top: 4mm; margin-bottom: 5mm; }
.fin-cell { padding: 3mm; background: var(--light); border-radius: 2px; text-align: center; }
...
.fin-name { font-size: 6pt; font-weight: 700; letter-spacing: 0.08em; text-transform: uppercase; color: var(--brand); margin-bottom: 1mm; }
.fin-amt { font-family: var(--serif); font-size: 10pt; font-weight: 700; color: var(--dark); line-height: 1.1; }
.fin-desc { font-size: 5.5pt; color: var(--muted); margin-top: 1mm; }
.fin-table { width: 100%; border-collapse: collapse; }
.fin-table th { font-size: 6.5pt; font-weight: 700; letter-spacing: 0.1em; text-transform: uppercase; color: var(--muted); padding: 1.5mm 0; border-bottom: 2px solid var(--ink); text-align: left; }
.fin-table th:last-child { text-align: right; }
.fin-table td { padding: 1.5mm 0; font-size: 7.5pt; border-bottom: 1px solid var(--rule); }
...
.fin-table tr.sub td { padding-left: 5mm; color: var(--muted); font-size: 7pt; }
.fin-table tr.total-row td { font-weight: 700; font-size: 8.5pt; color: var(--ink); border-top: 2px solid var(--ink); border-bottom: none; }

.kpi-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 6mm; margin-top: 4mm; }
.kpi-cell { padding: 3.5mm; border: 1px solid var(--rule); border-radius: 2px; }
.kpi-cell h4 { font-size: 7pt; }
.kpi-list li { padding: 1.2mm 0 1.2mm 4mm; border-bottom: 1px solid var(--rule); font-size: 7pt; color: var(--ink); position: relative; }
...
<div class="content-page">
  ...
  <h2>Governance</h2>
  <p style="margin-bottom:4mm">ODISSEI is the Dutch National Infrastructure that brings together sensitive data, computing resources and expertise to drive collaborative social science and economic research. It now consists of <strong>45 member organisations</strong>, with 31 already committed to membership through 2031.</p>
...
<div class="content-page">
  ...
  <p style="margin-bottom:4mm">Selected publications enabled by ODISSEI's data infrastructure in 2025, demonstrating the breadth and policy relevance of research conducted using ODISSEI facilities.</p>
